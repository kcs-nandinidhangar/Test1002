...
public class PurchaseHistoryReport {
	...
	private static TestCaseCategory PurchaseHistoryReport = TestCaseCategory.PURCHASE_HISTORY_REPORTS_DETAILS;
	...
	void testGetGlobalSalesOrderHeaderAPI(String testCaseNumber, String description, String exceptedResult,
		...
		LocalStorageContext.addTestInformation(testCaseNumber, PurchaseHistoryReport.toString());
			...
			ResponseBody<?> caraResponseBody = given()
					...
					.get("https://test.cara.ashleyretail.com/cara/customer/PurchaseHistoryReport?" + inputData2).getBody();
			...
			ResponseBody<?> cdpResponseBody = given()
					...
					.get("https://stage.cara.ashleyretail.com/cara/customer/PurchaseHistoryReport?" + inputData).getBody();
			/* CDP API Request End */
			/* Test Case Modification end from here */			
			
			TestUtility.updateActualAndExpectedResult(testCaseNumber, PurchaseHistoryReport.toString(),
	...
	private static Stream<Arguments> testGetGlobalSalesOrderHeaderAPICategory() {
		List<TestCase> testCases = LocalStorageContext
				.getTestCasesForCategory(PurchaseHistoryReport.toString());
