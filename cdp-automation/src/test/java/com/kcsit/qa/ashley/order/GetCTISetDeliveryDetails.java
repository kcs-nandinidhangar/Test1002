...
public class GetCTISetDeliveryDetails {
	/** The test case category. */
	private static TestCaseCategory CTISetDeliveryDetails = TestCaseCategory.CTI_SET_DELIVERY_DETAILS;
	...
	void testGetGlobalSalesOrderHeaderAPI(String testCaseNumber, String description, String exceptedResult,
		...
		LocalStorageContext.addTestInformation(testCaseNumber, CTISetDeliveryDetails.toString());
			...
			ResponseBody<?> caraResponseBody = given()
					...
					.get("https://test.cara.ashleyretail.com/cti-set-delivery-date?" + inputData).getBody();
			...
			ResponseBody<?> cdpResponseBody = given()
					...
					.get("http://cdp.stage.ashleyretail.com/sfsc/ivr/cti-set-delivery-date?" + inputData).getBody();
			/* CDP API Request End */
			/* Test Case Modification end from here */			
			
			TestUtility.updateActualAndExpectedResult(testCaseNumber, CTISetDeliveryDetails.toString(),
	...
	private static Stream<Arguments> testGetGlobalSalesOrderHeaderAPICategory() {
		List<TestCase> testCases = LocalStorageContext
				.getTestCasesForCategory(CTISetDeliveryDetails.toString());
