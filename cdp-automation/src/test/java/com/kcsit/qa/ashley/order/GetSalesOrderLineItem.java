...
public class GetSalesOrderLineItem {
	/** The test case category. */
	private static TestCaseCategory SalesOrderLineItemSearch = TestCaseCategory.SALES_ORDER_LINE_SEARCH;
	...
	void testGetGlobalSalesOrderHeaderAPI(String testCaseNumber, String description, String exceptedResult,
		...
		LocalStorageContext.addTestInformation(testCaseNumber, SalesOrderLineItemSearch.toString());
		...
		ResponseBody<?> caraResponseBody = given()
				...
				.get("https://test.cara.ashleyretail.com/odata/SalesOrderLineItems?" + inputData2).getBody();
		...
		ResponseBody<?> cdpResponseBody = given()
				...
				.get("https://cdp.stage.ashleyretail.com/transaction/odata/SalesOrderLineItems?" + inputData).getBody();
		/* CDP API Request End */
		/* Test Case Modification end from here */

		TestUtility.updateActualAndExpectedResult(testCaseNumber, SalesOrderLineItemSearch.toString(), caraResponseBody,
				cdpResponseBody);
	...
	private static Stream<Arguments> testGetGlobalSalesOrderHeaderAPICategory() {
		List<TestCase> testCases = LocalStorageContext.getTestCasesForCategory(SalesOrderLineItemSearch.toString());
