...
public class GetVOIPConfirmationget {
	/** The test case category. */
	private static TestCaseCategory VOIPConfirmationGet = TestCaseCategory.VOIP_CONFIRMATION_GET_SEARCH;
	...
	void testGetGlobalSalesOrderHeaderAPI(String testCaseNumber, String description, String exceptedResult,
		...
		LocalStorageContext.addTestInformation(testCaseNumber, VOIPConfirmationGet.toString());
		...
		ResponseBody<?> caraResponseBody = given()
				...
				.get("https://test.cara.ashleyretail.com/api/voip-confirmation?" + inputData).getBody();
		...
		ResponseBody<?> cdpResponseBody = given()
				...
				.get("https://cdp.stage.ashleyretail.com/sfsc/ivr/odata/voip-confirmation?" + inputData).getBody();
		/* CDP API Request End */
		/* Test Case Modification end from here */

		TestUtility.updateActualAndExpectedResult(testCaseNumber, VOIPConfirmationGet.toString(), caraResponseBody,
				cdpResponseBody);
	...
	private static Stream<Arguments> testGetGlobalSalesOrderHeaderAPICategory() {
		List<TestCase> testCases = LocalStorageContext.getTestCasesForCategory(VOIPConfirmationGet.toString());
