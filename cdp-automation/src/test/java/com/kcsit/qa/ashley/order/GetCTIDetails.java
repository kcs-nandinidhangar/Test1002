...
public class GetCTIDetails {
	/** The test case category. */
	private static TestCaseCategory CTIDetails = TestCaseCategory.CTI_DETAILS;
	...
	void testGetGlobalSalesOrderHeaderAPI(String testCaseNumber, String description, String exceptedResult,
		...
		LocalStorageContext.addTestInformation(testCaseNumber, CTIDetails.toString());
			...
			ResponseBody<?> caraResponseBody = given()
					...
					.get("https://test.cara.ashleyretail.com/odata/CTI?" + inputData).getBody();
			...
			ResponseBody<?> cdpResponseBody = given()
					...
					.get("https://stage.cara.ashleyretail.com/odata/CTI?" + inputData).getBody();
			/* CDP API Request End */
			/* Test Case Modification end from here */			
			
			TestUtility.updateActualAndExpectedResult(testCaseNumber, CTIDetails.toString(),
	...
	private static Stream<Arguments> testGetGlobalSalesOrderHeaderAPICategory() {
		List<TestCase> testCases = LocalStorageContext
				.getTestCasesForCategory(CTIDetails.toString());
