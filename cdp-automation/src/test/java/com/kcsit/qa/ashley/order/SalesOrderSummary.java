...
public class SalesOrderSummary {
	...
	private static TestCaseCategory SalesOrderSummary = TestCaseCategory.SALES_ORDER_SUMMARY_SEARCH;
	...
	void testGetGlobalSalesOrderHeaderAPI(String testCaseNumber, String description, String exceptedResult,
		...
		LocalStorageContext.addTestInformation(testCaseNumber, SalesOrderSummary.toString());
			...
			ResponseBody<?> caraResponseBody = given()
					...
					.get("https://test.cara.ashleyretail.com/SalesOrderSummary?" + inputData2).getBody();
			...
			ResponseBody<?> cdpResponseBody = given()
					...
					.get("https://stage.cara.ashleyretail.com/salesorderdetail/SalesOrderSummary?" + inputData).getBody();
			/* CDP API Request End */
			/* Test Case Modification end from here */			
			
			TestUtility.updateActualAndExpectedResult(testCaseNumber, SalesOrderSummary.toString(),
	...
	private static Stream<Arguments> testGetGlobalSalesOrderHeaderAPICategory() {
		List<TestCase> testCases = LocalStorageContext
				.getTestCasesForCategory(SalesOrderSummary.toString());
