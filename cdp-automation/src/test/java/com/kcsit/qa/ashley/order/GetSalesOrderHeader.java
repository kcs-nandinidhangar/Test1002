...
public class GetSalesOrderHeader {
	...
	private static TestCaseCategory SalesOrderHeaderSearch = TestCaseCategory.SALES_ORDER_HEADER_SEARCH;
	...
	void testGetGlobalSalesOrderHeaderAPI(String testCaseNumber, String description, String exceptedResult,
		...
		LocalStorageContext.addTestInformation(testCaseNumber, SalesOrderHeaderSearch.toString());
		...
		ResponseBody<?> caraResponseBody = given()
				...
				.get("https://test.cara.ashleyretail.com/odata/SalesOrderHeaders?" + inputData2).getBody();
		...
		ResponseBody<?> cdpResponseBody = given()
				...
				.get("https://cdp.stage.ashleyretail.com/transaction/odata/SalesOrderHeaders?" + inputData).getBody();
		/* CDP API Request End */
		/* Test Case Modification end from here */

		TestUtility.updateActualAndExpectedResult(testCaseNumber, SalesOrderHeaderSearch.toString(), caraResponseBody,
				cdpResponseBody);
	...
	private static Stream<Arguments> testGetGlobalSalesOrderHeaderAPICategory() {
		List<TestCase> testCases = LocalStorageContext.getTestCasesForCategory(SalesOrderHeaderSearch.toString());
