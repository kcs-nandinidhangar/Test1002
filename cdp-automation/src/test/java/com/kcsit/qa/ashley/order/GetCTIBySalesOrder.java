...
public class GetCTIBySalesOrder {
	/** The test case category. */
	private static TestCaseCategory CTIBySODetails = TestCaseCategory.CTI_BY_SO_DETAILS;
	...
	void testGetGlobalSalesOrderHeaderAPI(String testCaseNumber, String description, String exceptedResult,
		...
		LocalStorageContext.addTestInformation(testCaseNumber, CTIBySODetails.toString());
			...
			ResponseBody<?> caraResponseBody = given()
					...
					.get("https://test.cara.ashleyretail.com/CTI-By-Sale?" + inputData).getBody();
			...
			ResponseBody<?> cdpResponseBody = given()
					...
					.get("https://stage.cara.ashleyretail.com/cti-by-sale?" + inputData).getBody();
			/* CDP API Request End */
			/* Test Case Modification end from here */			
			
			TestUtility.updateActualAndExpectedResult(testCaseNumber, CTIBySODetails.toString(),
	...
	private static Stream<Arguments> testGetGlobalSalesOrderHeaderAPICategory() {
		List<TestCase> testCases = LocalStorageContext
				.getTestCasesForCategory(CTIBySODetails.toString());
