...
public class GetCTIGetDeliveryDetails {
	/** The test case category. */
	private static TestCaseCategory CTIGetDeliveryDetails = TestCaseCategory.CTI_GET_DELIVERY_DETAILS;
	...
	void testGetGlobalSalesOrderHeaderAPI(String testCaseNumber, String description, String exceptedResult,
		...
		LocalStorageContext.addTestInformation(testCaseNumber, CTIGetDeliveryDetails.toString());
			...
			ResponseBody<?> caraResponseBody = given()
					...
					.get("https://test.cara.ashleyretail.com/cti-delivery-dates?" + inputData).getBody();
			...
			ResponseBody<?> cdpResponseBody = given()
					...
					.get("https://stage.cara.ashleyretail.com/cti-delivery-dates?" + inputData).getBody();
			/* CDP API Request End */
			/* Test Case Modification end from here */			
			
			TestUtility.updateActualAndExpectedResult(testCaseNumber, CTIGetDeliveryDetails.toString(),
	...
	private static Stream<Arguments> testGetGlobalSalesOrderHeaderAPICategory() {
		List<TestCase> testCases = LocalStorageContext
				.getTestCasesForCategory(CTIGetDeliveryDetails.toString());
